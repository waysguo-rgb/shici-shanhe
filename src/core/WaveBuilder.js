...
// 贴地共享几何: PlaneGeometry 预先 rotateX(-90°) 烘进顶点, mesh 默认平躺水面.
// 替代 Sprite billboard — 陡俯视角下 Sprite 会仰成"立着朝天的白片"穿帮, 平躺 mesh 真正画在水面上.
// 注意: rotateX(-90°) 后 geometry 的 Y 维映射到世界 Z, 故 mesh.scale 用 (宽X, 1, 深Z).
const _wavePlaneGeo = new THREE.PlaneGeometry(1, 1).rotateX(-Math.PI / 2);
  ...
  for (let r = 0; r < rows; r++) {
    for (let co = 0; co < cols; co++) {
      ...
      layers.forEach((L, idx) => {
        ...
        const sp = new THREE.Mesh(_wavePlaneGeo, mat);
        const jitter = 0.88 + Math.random() * 0.24;
        const sw = baseW * L.scale * jitter;
        const sh = baseH * L.scale * jitter;
        // 一半镜像翻转 — 把符号烘进 baseW, 这样 animation 重写 scale 时也能保留
        const flip = Math.random() < 0.5 ? -1 : 1;
        const sw_signed = sw * flip;
        sp.scale.set(sw_signed, 1, sh);   // 平躺: 宽放 X, 深放 Z (geometry 已 rotateX)
        sp.position.set(px, L.dy, pz + L.zOff);   // 贴水面, 不再抬半高 (平躺无需)
  ...
  for (let i = 0; i < BD_MAIN.length; i++) {
    ...
    for (let s = 0; s < nP; s++) {
      ...
      for (let L = 0; L < nLayers; L++) {
        ...
        // 一半镜像翻转, 朝向多样化
        const flip = Math.random() < 0.5 ? -1 : 1;
        sp.scale.set(bW * flip, 1, bH);   // 平躺: 宽放 X, 深放 Z
...
export function animateSea(t) {
  for (let i = 0; i < coastWaveData.length; i++) {
    ...
    // scale 摆动 ±12%/±25% → ±5%/±6%: 几乎不缩放, 去掉"搏动"感 (深度放 Z, 平躺 mesh)
    d.sp.scale.set(d.bW * (1 + crest * 0.05), 1, d.bH * (1 + crest * 0.06));
    // opacity 近 3 倍跳变 → 0.82±0.06: 透明度突变是肉眼最敏感的廉价信号, 压成微呼吸
    d.sp.material.opacity = d.bOp * (0.82 + sv * 0.06);
    d.sp.rotation.y = Math.sin(ph * 0.4) * 0.03;   // 绕水面法线转 (平躺 mesh, 替代 material.rotation)
  }
}
